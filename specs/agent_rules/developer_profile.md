...
- JSON per API
- HTML per pagine UI
- Codici HTTP appropriati (200, 201, 400, 404, 500)

### Testing

#### Struttura
- Un file test per modulo
- Naming: `test_<module>.py`
- Fixture per setup/teardown

#### Coverage
- Target minimo: 80%
- Testare casi happy path E edge cases
- Mockare servizi esterni (Ollama, requests)
...
- `WARNING`: Qualcosa non va ma non blocca
- `ERROR`: Errore che richiede attenzione

#### Best Practices
- Loggare inizio/fine operazioni importanti
- Includere contesto (ID, parametri)
- Non loggare dati sensibili
- Usare struttura (JSON) per log machine-readable

### Linting e Formattazione

#### Regole
- **ruff**: Linting (linee, import, naming)
- **black**: Formattazione codice
- **mypy**: Type checking (opzionale, target strict)
...
### Prima di Iniziare
1. Leggere specs esistenti
2. Verificare branch attuale
3. Aggiornare da main se necessario

### Durante lo Sviluppo
1. Seguire TDD quando possibile
2. Commit piccoli e frequenti
3. Mai committare codice non testato

### Dopo lo Sviluppo
1. Eseguire full test suite
2. Verificare coverage
3. Run linting/formatter
4. Aggiornare documentation

## Eccezioni e Deroghe

Le regole sopra possono essere derogate solo se:
1. C'è un motivo tecnico valido documentato
2. Il comportamento è stato discusso e approvato
3. La deroga è temporanea e pianiificata per essere rimossa

## Revisione Codice

Ogni PR deve essere reviewato da almeno una persona prima del merge. Review deve verificare:
